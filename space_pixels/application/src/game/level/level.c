...
static void Level_updateLevel1(Level *self)
{
    ...
    {
    case 0:
        enemy = Enemy_create(scene, ENEMY_TYPE_BATTLECRUISER, Vec2_set(9.0f, 4.5f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_FRIGATE, Vec2_set(10.0f, 3.0f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_FRIGATE, Vec2_set(10.0f, 6.0f));
        LevelScene_addEnemy(scene, enemy);
        break;


        //* TODO 19: Ajouter une vague d'ennemis
    case 1:
        enemy = Enemy_create(scene, ENEMY_TYPE_SCOUT, Vec2_set(11.0f, 1.5f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_TORPEDO, Vec2_set(10.0f, 3.0f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_BATTLECRUISER, Vec2_set(9.0f, 4.5f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_TORPEDO, Vec2_set(10.0f, 6.0f));
        LevelScene_addEnemy(scene, enemy);
        enemy = Enemy_create(scene, ENEMY_TYPE_SCOUT, Vec2_set(11.0f, 7.5f));
        LevelScene_addEnemy(scene, enemy);
        break;
...
Level *Level_create(LevelScene *scene, int levelID)
{
    ...
    case LEVEL_1:
     ...
     // TODO 20 : LEVEL 2
    case LEVEL_2:
        self->m_backgroundID = SPRITE_BACKGROUND_PURPLE_NEBULA;
        break;
    }
