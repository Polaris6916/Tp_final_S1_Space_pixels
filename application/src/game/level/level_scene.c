...
void LevelScene_updateEngine(LevelScene *self)
{
    ...
    for (int i = 0; i < BULLET_CAPACITY; i++)
    {
        ...
        else
        {
            ...
            if (bullet->m_type == BULLET_FRIGATE)
            {
                for (int j = 0; j < BULLET_CAPACITY; j++)
                {
                    Bullet* autre_projectile = self->m_bullets[j];
                    if (autre_projectile == NULL) continue;
                    if (bullet == autre_projectile) continue;
                    if (autre_projectile->m_type == BULLET_FRIGATE) continue;


                    float dist = Vec2_distanceSquared(bullet->m_position, autre_projectile->m_position);
                    if (dist < 2 * 2 * PIX_TO_WORLD)
                    {
                        Bullet_setState(bullet, BULLET_STATE_HIT_TARGET);
                        Bullet_setState(autre_projectile, BULLET_STATE_HIT_TARGET);
                        break;
    ...
    for (int i = 0; i < ENEMY_CAPACITY; i++)
    {
        ...
        Enemy_update(enemy);
        int nb_player = self->m_playerCount;
        for (int j = 0; j < nb_player; j++)
        {
            Player* player = self->m_players[j];
            if (player == NULL) continue;
            if (player->m_type == SPRITE_PLAYER_DASH)
            {
                float dist = Vec2_distanceSquared(player->m_position, enemy->m_position);
                float radius = player->m_radius + enemy->m_radius;
                if (dist < radius * radius)
                {
                    PlayerInput playerInput = Player_getInput(player);
